...
| Tower signing of ledger entries | **IMPLEMENTED (v0.9.16)** — `entry.append` and `session.commit` sign entries via tower signer `crypto.sign_ed25519` when `TOWER_SIGNER_SOCKET` is set (deprecated `BEARDOG_SOCKET` fallback preserved). Signature stored in entry metadata (`tower_signature`, `tower_signature_alg`). Standalone mode produces unsigned entries (backward-compatible). |
