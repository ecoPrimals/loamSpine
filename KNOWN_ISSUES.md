...
| BTSP Phase 3 — `btsp.negotiate` (null cipher) | **IMPLEMENTED** — `btsp.negotiate` JSON-RPC method handler returns `cipher: "null"` (plaintext fallback). primalSpring Phase 3 clients no longer get `METHOD_NOT_FOUND`. Full encrypted framing (ChaCha20-Poly1305 AEAD) deferred until the BTSP provider API exports session key material for server-side HKDF derivation. Zero new crypto dependencies. |
