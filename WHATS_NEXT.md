...
- **June 28, 2026** — **Wave 128 Convergence: Provenance Depth + Debt Cleanup**: Evolved `ProvenanceLink` with `depth` field for Nest provenance depth tracking (ledger → 5+). `get_provenance_chain()` now sorts by timestamp/index and assigns depth, matching `PublicChainAnchor` (chain-anchored) and `CertificateMint` (certified-by) in addition to DataAnchor, SessionCommit, BraidCommit. 6-link cross-spine 5+ depth test added. BearDog → generic BTSP provider terminology cleaned across all production doc comments (wire.rs, handshake.rs, phase3.rs, provider_client.rs, btsp_ops.rs, service/mod.rs). Coverage push: 11 new env_resolution tests (family_seed/tower_signer_socket fallback chains, key constant validation), 4 new crypto_provider tests (error response, verify_entry delegation, request counter). 1,669 tests, 199 source files.
...
- **Tests**: 1,490 (+4). All gates green (clippy, fmt, deny). Ionic bond blocker fully resolved — both negotiate handler and transport loop verified.
...
- **~~Showcase demos~~** — **FOSSILIZED** (May 25, 2026): Wave 49 — archived to `ecoPrimals/fossilRecord/primals/loamSpine/showcase_wave49/`. Active validation via `infra/benchScale/validate_roundtrip.sh` (52 validations, 47 methods, 20 phases).
