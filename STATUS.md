...
| [API_SPECIFICATION.md](specs/API_SPECIFICATION.md) | COMPLETE | 42 JSON-RPC methods (semantic naming), tarpc server. Spec updated to match implementation. |
...
| AGPL-3.0-or-later | PASS | SPDX headers on all 184 source files (+ 3 fuzz targets) |
...
All 42 methods have stability annotations in `capabilities.list` response:
- **stable**: spine, entry, certificate, proof, anchor, session, braid, bonding, btsp, lifecycle, health, auth, primal (38 methods)
...
- **All gates green**: `cargo fmt` PASS, `cargo clippy --all-targets --all-features -D warnings` PASS (0 warnings), `cargo doc` PASS (0 warnings), `cargo test` PASS (1,490 tests, 0 failures), `cargo deny check` PASS.
