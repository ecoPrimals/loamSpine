...
async fn test_provenance_chain_depth_five_plus() {
    ...
    // Verify chain is ordered by depth
    ...
    // Verify cross-spine provenance (links from both spines)
    ...
    // Final depth value should be >= 5 (0-indexed, so last link.depth >= 5)
...
async fn test_provenance_chain_includes_certificate_mint() {
    ...
    // Mint a certificate — we'll search by the entry hash of the mint itself
...
async fn test_provenance_chain_deterministic_ordering() {
    ...
    // Append 3 entries referencing the same content hash
    for _ in 0..3 {
        service
            .append_entry(
                spine_id,
    ...
    // Verify strictly increasing depth
    for (i, link) in chain.iter().enumerate() {
        let expected = u32::try_from(i).unwrap_or(u32::MAX);
        assert_eq!(link.depth, expected);
    }
    // Verify monotonic index ordering within same spine
