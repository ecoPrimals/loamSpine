...
async fn redb_spine_storage_crud() {
    ...
    storage
        ...
        .await
        .unwrap_or_else(|_| unreachable!());
    assert_eq!(storage.spine_count().unwrap(), 1);
    ...
    storage
        ...
        .await
        .unwrap_or_else(|_| unreachable!());
    assert_eq!(storage.spine_count().unwrap(), 0);
...
async fn redb_concurrent_operations() {
    ...
    assert_eq!(storage.spines.spine_count().unwrap(), 10);
}
...
async fn redb_save_and_retrieve_multiple_entries() {
    ...
    let entries = entry_storage
        ...
        .unwrap();
    assert_eq!(entries.len(), 5);
    assert_eq!(entry_storage.entry_count().unwrap(), 5);
}
...
async fn redb_storage_open_with_base_path() {
    ...
    assert_eq!(storage.spines.spine_count().unwrap(), 0);
    assert_eq!(storage.entries.entry_count().unwrap(), 0);
    assert_eq!(storage.certificates.certificate_count().unwrap(), 0);
}
...
async fn redb_entry_sequential_saves() {
    ...
    assert_eq!(storage.entry_count().unwrap(), 10);
    let entries = storage
...
async fn redb_spine_count_after_multiple_operations() {
    ...
    for i in 0..5 {
        ...
        storage.save_spine(&spine).await.unwrap();
        assert_eq!(storage.spine_count().unwrap(), i + 1);
    }

    assert_eq!(storage.spine_count().unwrap(), 5);
}
...
async fn redb_entry_count_after_multiple_operations() {
    ...
    for i in 0..7 {
        ...
        assert_eq!(
            storage.entry_count().unwrap(),
            usize::try_from(i + 1).unwrap_or(0)
        );
    }

    assert_eq!(storage.entry_count().unwrap(), 7);
}
