...
impl SyncProtocol for SyncEngine {
    async fn push_entries(
        ...
        // Push by reference first (zero-copy network path)
        let push_result = self.push_to_peer(&endpoint, spine_id, &entries).await;

        // Move into local queue (ownership transfer, no clone)
        self.spine_states
            .write()
            .await
            .entry(spine_id)
...
///
/// Absorbs the ecosystem resilience pattern for outbound IPC. The inner
/// `SyncEngine` handles wire protocol; this wrapper handles failure isolation.
///
