...
//!
//! Enables loamSpine to serve multiple RPC protocols (JSON-RPC + tarpc) on a
//! single UDS socket. The client sends `PROTOCOLS: tarpc,jsonrpc\n`, the
//! server selects the best mutual match and responds `PROTOCOL: tarpc\n`.
//! No negotiation = JSON-RPC (full backward compatibility).
//!
...
pub enum IpcProtocol {
    /// tarpc binary (length-delimited + JSON serde). Sub-ms, type-safe.
    Tarpc,
    /// JSON-RPC 2.0 (newline-delimited). Default fallback.
    JsonRpc,
}
...
pub fn parse_protocols_line(line: &str) -> Option<Vec<IpcProtocol>> {
    let rest = line.trim().strip_prefix("PROTOCOLS:")?;
    let protocols: Vec<IpcProtocol> = rest
        .split(',')
        .filter_map(|s| IpcProtocol::from_wire_name(s.trim()))
        .collect();
    Some(protocols)
}
...
mod tests {
    ...
    fn wire_name_roundtrip() {
        assert_eq!(
            IpcProtocol::from_wire_name("tarpc"),
            Some(IpcProtocol::Tarpc)
        );
        assert_eq!(
            IpcProtocol::from_wire_name("jsonrpc"),
    ...
    fn parse_protocols_line_valid() {
        ...
        let protos = parse_protocols_line("PROTOCOLS: tarpc\n").unwrap();
        assert_eq!(protos, vec![IpcProtocol::Tarpc]);
    }

    #[test]
    fn parse_protocols_line_invalid() {
        assert!(parse_protocols_line("{\"jsonrpc\":\"2.0\"}").is_none());
        assert!(parse_protocols_line("").is_none());
        assert!(parse_protocols_line("PROTOCOL: tarpc\n").is_none());
    }

    #[test]
    fn select_protocol_client_preference() {
    ...
    #[test]
    fn format_protocol_response_values() {
        assert_eq!(
    ...
    async fn negotiate_roundtrip_jsonrpc_only() {
        ...
        let selected = negotiate_client(&mut client, &[IpcProtocol::JsonRpc])
            .await
            .unwrap();
