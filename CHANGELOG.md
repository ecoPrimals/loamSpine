...
- **`get_provenance_chain()` matches `SessionCommit`**: Provenance chain queries now recognize `SessionCommit` entries by `merkle_root`, returning relationship `committed-from`. Previously only `DataAnchor` and `BraidCommit` entries were matched.
...
### Changed (April 23, 2026)

- **BTSP Step 3→4 verification relay fixed**: All three BearDog relay calls (`btsp.session.create`, `btsp.session.verify`, `btsp.negotiate`) aligned with `beardog_types::btsp::rpc` contract. `create` now sends `family_seed` (base64-encoded from `FAMILY_SEED`/`BEARDOG_FAMILY_SEED` env) instead of `family_seed_ref: "env:..."` — challenge generated by BearDog, not loamSpine. `verify` uses `session_token`/`response`/`preferred_cipher` instead of `session_id`/`client_response`/`server_ephemeral_pub`/`challenge`. `negotiate` uses `session_token`/`cipher` instead of `session_id`/`preferred_cipher`/`bond_type`. Response types aligned: `SessionCreateResult.session_token`+`challenge`, `NegotiateResult.accepted`. `generate_challenge()` removed (BearDog is challenge authority). `resolve_family_seed()` added. 5 new tests. Same pattern as sweetGrass April 22 fix.
...
- **Deep debt audit clean**: All 10 dimensions pass: zero files >800L prod, zero `unsafe`, zero `todo!()`/`FIXME`/`HACK`, zero hardcoded primal names in prod, all mocks cfg-gated, `cargo deny` clean, 4 `#[allow]` justified (2× tarpc `wildcard_imports`, 2× feature-conditional `unused_async`), all `#[expect]` with `reason`, clippy zero warnings (default + all-features), `cargo fmt` clean. Lockfile refreshed. 1,499 tests.
- **`#[allow]` count reconciled**: README/STATUS corrected to 4 (2× tarpc `wildcard_imports` + 2× feature-conditional `unused_async`).
...
### Changed (April 15, 2026)

- **BTSP HandshakeComplete wire format fixed**: `HandshakeComplete` now includes `"status":"ok"` field required by primalSpring's wire protocol. Without this discriminator field, primalSpring could not identify the message as a successful completion and timed out waiting. Both length-prefixed and NDJSON handshake paths updated. 1 new wire-format assertion test (1,503 total). Resolves primalSpring Phase 45c "incomplete handshake" gap — `ledger` capability should now report PASS in guidestone.
