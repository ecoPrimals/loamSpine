...
- **Deep debt audit clean**: All 10 dimensions pass: zero files >800L prod, zero `unsafe`, zero `todo!()`/`FIXME`/`HACK`, zero hardcoded primal names in prod, all mocks cfg-gated, `cargo deny` clean, 2 `#[allow]` justified (feature-conditional async), all `#[expect]` with `reason`, clippy zero warnings (default + all-features), `cargo fmt` clean. Lockfile refreshed. 1,499 tests.
- **`#[allow]` count corrected**: README/STATUS updated from stale "4" to actual "2" — the 2 tarpc `wildcard_imports` allows were cleaned up previously but docs were not updated.
