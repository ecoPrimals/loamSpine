...
- **High Coverage**: Aim for 90%+ line coverage (current: 92.26% line, 1,787 tests)
...
# Test (1,787 tests, all concurrent)
...
refactor(service): Split large test files by domain
```
...
| Version | 0.9.16 |
| Edition | 2024 |
| Tests | 1,787 (all concurrent, ~3s, zero flaky) |
...
| SPDX Headers | All 211 source files |
| Source files | 211 `.rs` across workspace (+ 3 fuzz targets) |
